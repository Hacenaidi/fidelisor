<x-app>
    <title>FIDELISOR - Validation Gift</title>
    ...
    <div class="card card-body border-0 shadow mb-5 mt-5">
        ...
        <div class="d-flex flex-row row">
            ...
            <div class="p-2 mt-2 col-xl-1 col-md-2 col-sm-2">
              ...
              <div class="p-2 col-xl-2 col-md-3 col-sm-3">
                ...
                <li><a class="dropdown-item" href="{{route("gifts.validation",$s->id)}}">{{$s->nomStore}}</a></li>
                @endforeach
                  </ul>
                </div>
            </div>  
       ...
       {{session("incorrect")}}
                ...
                >
                    <thead>
                        <tr>
                            ...
                            <th scope="col">GIFT CARD Name</th>
                    ...
                    <tbody>
                        @if($purchase != null)
                        <tr class="">
                            <td>{{$purchase->profile->nom}}</td>
                            <td>{{$purchase->profile->email}}</td>
                            <td>{{$purchase->variation->gift->namecard}}</td>
                            <td>{{$purchase->variation->variation_name}}</td>
                            <td>{{$purchase->quantite}}</td>
                            <td>{{$purchase->total_price." ".$purchase->variation->gift->currency}}</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
        ...
        <div style="margin: 0 auto;height:25rem">
            <form action="{{route("gifts.validated",$store->id)}}" method="post">
                @csrf
                <div class="d-flex">
                    <div class="p-2"><input
                        type="text"
